...
```bash
# Export ATC-organized GraphML subnetworks from shRNA compound–target data
python newt/scripts/export_graphml_ATC_subnetworks_v5_shRNA.py

# Optional: post-process to copy target lists
cd ct_network_exports_ATC_subnetworks_shRNA
python copy_graphml_targets.py
```

**What it does:**  
Builds hierarchical ATC-based subnetworks (levels 1–3) and saves both “top‑10” filtered and full GraphMLs with matching CSVs.

- Input folder: `./ct_network_exports_shRNA`  
  (expects CSVs with columns: `Query Pert ID, Compound Name, Symbol, Probability`)
- WHO ATC file: `./data/WHO_ATC_DDD_2024-07-31.csv` (editable via `ATC_FILE`)
- Output folder: `./ct_network_exports_ATC_subnetworks_shRNA/`

**Outputs per subnetwork:**
- `*_graphml` → filtered top‑10 (and full, at level‑3)
- `*_details.csv` → with `[atc_code, atc_name, drug_name, compound, target]`
- `*_targets.csv` → deduplicated target list (no header)

**Example folder structure (abridged):**
```
        ...
        └── A01_Stomatological_preparations/
            ...
            └── subgroups/
                └── A01A_Stomatological_agents/
                    ├── A01A_Stomatological_agents.graphml
                    ├── A01A_Stomatological_agents_full.graphml
